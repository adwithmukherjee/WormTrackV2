...
framesforposlist = framesforposlist[0:10] #TEMPORARY UNTIL GETPOSLIST IS FASTER
#INSTEAD: LOAD IN CSV JOHN IS MAKING?

ImgPath = "ClassifierInput"
frames = glob.glob(os.path.join(ImgPath, '*.jpg'))
frames.sort()
frames = frames[0:10] #TEMPORARY UNTIL GETPOSLIST IS FASTER
...
for f in range(0,len(positionlist)):
    x = 2*((positionlist[f][0])/512) - 1
    y = 2*((positionlist[f][1])/400) - 1
